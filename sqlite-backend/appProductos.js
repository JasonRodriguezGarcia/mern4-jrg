...
// import { searchProducto } from './models/productoModel.js';
    ...
    try {
      switch (choice) {
        ...
        case '5': {
            ...
            const productos = await searchProducto(searchName);
            console.log(productos);
            if (productos.length === 0) console.log('No products found.');
            else {
                productos.forEach(p => {
        ...
        case '6': {
            const lowStockQuantity = 3
            const productos = await lowStock(lowStockQuantity);
            console.log(productos);
            if (productos.length === 0) console.log('No products found.');
            else {
                productos.forEach(p => {
